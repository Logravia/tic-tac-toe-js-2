...
const board = (function () {
  ...
  const checkDiagonalWin = function () {
    let leftDiag = [state[0][0], state[1][1], state[2][2]];
    let rightDiag = [state[0][2], state[1][1], state[2][0]];
    return checkHorizontalWin([leftDiag, rightDiag]);
  };

  const isVictory = function () {
    return (
      checkHorizontalWin(state) || checkVerticalWin() || checkDiagonalWin()
    );
  };
